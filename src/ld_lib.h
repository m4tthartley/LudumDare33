...
void LD_RDrawSpriteWithColor (ld_sprite Sprite, f32 X, f32 Y, f32 Scale, color Color)
{
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, Sprite.Texture.ID);
	glColor4f(Color.R, Color.G, Color.B, Color.A);
		...
		if (ImageHeader->BitCount == 8)
		{
				...
				Y++)
			{
					...
					X++)
				{
					...
					if (PixelColor == 0xFF191818)
					{
						PixelColor = 0x88191919;
					}
		...
		SoundIndex++)
	{
		...
		if (Sound->Playing)
		{
				...
				WriteIndex++)
			{
				...
				Samples[WriteIndex] += *((s16*)(Sound->Buffer + Sound->Cursor)) * 0.5f;
				Sound->Cursor += 2;
			}
